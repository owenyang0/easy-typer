...
def push_to_baidu(site, urls, token, bark_token):
    ...
    try:
        ...
        if "success" in result and result["success"]:
            print("成功推送url到百度.")
            bark_url = f"https://api.day.app/{bark_token}/typer.owenyang.top/✅成功url推送到百度."
            requests.get(bark_url)
        elif "error" in result:
            print("推送url到百度出现错误，错误信息为：", result["message"])
            bark_url = f"https://api.day.app/{bark_token}/typer.owenyang.top/❌推送url失败{result['message']}"
            requests.get(bark_url)
        else:
