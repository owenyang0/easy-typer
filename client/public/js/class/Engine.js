define(
   ...
   class Engine {
      constructor() {
         ...
         window.electronAPI.handlePaste((evt, val) => {
            console.log('val', val)
            this.paste(val);
            $('#pad').focus();
         })
         ...
         typingPad.onkeydown = e => {
            if (e.key === 'Tab' || ((e.metaKey || e.ctrlKey) && (/[nwefgplt]/.test(e.key)))) {
               // 消除一些默认浏览器快捷键的效果
               e.preventDefault();
      ...
      async paste (val) {
         const text = val || await navigator.clipboard.readText();
         // console.log(text);
         this.changeArticle(editor, text)
      }
      // 改变文章内容
      changeArticle(editor, content = '') {
         ...
         if (!!content) {
            const result = Utility.parseContent(content)
            console.log(result);

            this.config.identity = result.paragraphNO;
            this.config.articleType = 'customize';
            this.config.count = 'ALL'

            this.config.customizedTitle = result.title;
            this.config.customizedContent = result.content;
         ...
         console.log(articleName,this.config);
      ...
      getRecordById (id) {
         this.database.getRecordById(id)
      }
      // 清除记录
      clear(sender) {
      ...
      finish() {
         ...
         // update record
         this.record.identity = this.config.identity;
         ...
         // // 复制成绩到粘贴板
         // Utility.putGradeToClipboard(this.record);
         ...
         // 复制成绩到粘贴板
         Utility.putGradeToClipboard(this.record, this.keyCount);
