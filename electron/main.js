...
const { app, globalShortcut, ipcMain, BrowserWindow,BrowserView, clipboard } = require('electron')
...
const createWindow = () => {
    ...
    ipcMain.on('set-grade', (event, msg) => {
        ...
        console.log(msg)
        applescript.execString(sendingScript, (err, ret) => {
            if (err) {
...
app.whenReady().then(() => {
    ...
    const ret = globalShortcut.register('F4', () => {
        ...
        applescript.execString(retrivingScript, (err, ret) => {
            if (err) {
              // Something went wrong!
                console.log('err', err);
            }
